...
#define D_ADD(a,b,c) if (a > 0xffffffff - (c)) ++b; a += c;
#define L_ROTATE(a,b) (((a) << (b)) | ((a) >> (32-(b))))
#define R_ROTATE(a,b) (((a) >> (b)) | ((a) << (32-(b))))
#define EP0(x) (R_ROTATE(x,2) ^ R_ROTATE(x,13) ^ R_ROTATE(x,22))
#define EP1(x) (R_ROTATE(x,6) ^ R_ROTATE(x,11) ^ R_ROTATE(x,25))
#define SIG0(x) (R_ROTATE(x,7) ^ R_ROTATE(x,18) ^ ((x) >> 3))
#define SIG1(x) (R_ROTATE(x,17) ^ R_ROTATE(x,19) ^ ((x) >> 10))
...
void SHA256CHANGE(SHA256_c_x *c_x, uchar data[])
{
	uint h0, h1, h2, h3, h4, h5, h6, h7, i, j, t1, t2, m[64];
	...
	for (i = 0; i < 64; ++i) {
		t1 = h7 + EP1(h4) + CH(h4, h5, h6) + k[i] + m[i];
...
void SHA256OR(SHA256_c_x *c_x)
{
	c_x->d_le = 0;
	c_x->b_le[0] = 0;
	c_x->b_le[1] = 0;
	c_x->s_t[0] = 0x6a09e667;
	c_x->s_t[1] = 0xbb67ae85;
	c_x->s_t[2] = 0x3c6ef372;
	c_x->s_t[3] = 0xa54ff53a;
	c_x->s_t[4] = 0x510e527f;
	c_x->s_t[5] = 0x9b05688c;
	c_x->s_t[6] = 0x1f83d9ab;
	c_x->s_t[7] = 0x5be0cd19;
}

void SHA256VALIDATE(SHA256_c_x *c_x, uchar data[], uint len)
{
	for (uint i = 0; i < len; ++i) {
		c_x->data[c_x->d_le] = data[i];
		c_x->d_le++;
		if (c_x->d_le == 64) {
			SHA256CHANGE(c_x, c_x->data);
			D_ADD(c_x->b_le[0], c_x->b_le[1], 512);
			c_x->d_le = 0;
		}
	}
}

void SHA256Final(SHA256_c_x *c_x, uchar tag[])
{
	uint i = c_x->d_le;
	...
	if (c_x->d_le < 56) {
		...
		SHA256CHANGE(c_x, c_x->data);
		memset(c_x->data, 0, 56);
	}

	D_ADD(c_x->b_le[0], c_x->b_le[1], c_x->d_le * 8);
	c_x->data[63] = c_x->b_le[0];
	c_x->data[62] = c_x->b_le[0] >> 8;
	c_x->data[61] = c_x->b_le[0] >> 16;
	c_x->data[60] = c_x->b_le[0] >> 24;
	c_x->data[59] = c_x->b_le[1];
	c_x->data[58] = c_x->b_le[1] >> 8;
	c_x->data[57] = c_x->b_le[1] >> 16;
	c_x->data[56] = c_x->b_le[1] >> 24;
	SHA256CHANGE(c_x, c_x->data);

	for (i = 0; i < 4; ++i) {
		tag[i] = (c_x->s_t[0] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 4] = (c_x->s_t[1] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 8] = (c_x->s_t[2] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 12] = (c_x->s_t[3] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 16] = (c_x->s_t[4] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 20] = (c_x->s_t[5] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 24] = (c_x->s_t[6] >> (24 - i * 8)) & 0x000000ff;
		tag[i + 28] = (c_x->s_t[7] >> (24 - i * 8)) & 0x000000ff;
	}
}
string SHA256Algo(char* data) {
	...
	SHA256OR(&c_x);
	SHA256VALIDATE(&c_x, (unsigned char*)data, strLen);
	SHA256Final(&c_x, tag);
...
int main() {
char book_data[] = "[1] The beginning of the gospel of Jesus Christ, the Son of God."
...
string sha256Algo = SHA256Algo(book_data);
 cout  << sha256Algo << endl;
return 0;
}